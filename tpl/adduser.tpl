...
<form enctype="multipart/form-data" method="post" name="user_form" id="user_form">
    <div id="subject_family_container">
        <div class="section_header">Select Father
            <span class="x_box" id="father_x_box" onclick="hideUsers('father',1,0)"><img src="" width="16" /></span>
        </div>
        {foreach $father as $family_member}
            <div class="section_container">
                <div id="father" onclick="changeUserTile('father',1,0)">{include file="_user_tile.tpl"}</div>
                <div class="change_user" id="change_father"></div>
            </div>
        {/foreach}
        
        <div class="section_header">Select Mother
            <span class="x_box" id="mother_x_box" onclick="hideUsers('mother',0,{$family_member.user_id})"><img src="" width="16" /></span>
        </div>
        {foreach $mother as $family_member}
            <div class="section_container">
        ...
        <div class="section_header">Select Spouse
            <span class="x_box" id="spouse_x_box" onclick="hideUsers('spouse',1,0)"><img src="" width="16" /></span>
        </div>
        {foreach $spouse as $family_member}
            <div class="section_container">
                <div id="spouse" onclick="changeUserTile('spouse',1,0)">{include file="_user_tile.tpl"}</div>
                <div class="change_user" id="change_spouse"></div>
            </div>
        {/foreach}
    
        <div class="section_header">Add Picture</div>
        <div class="section_container">
            <img src="images/faces/no_image_male.jpg" width="100%" />
                    ...
                    <td>
                        <select name="dob_month">
                            {foreach $months as $month}
                                <option value="{$month.id}" {if $subject.dob|date_format:"%m" == $month.id} selected {/if}>{$month.month}</option>
                            {/foreach}
                        </select>
                        <select name="dob_date">
                            {foreach $dates as $date}
                                <option value="{$date.id}" {if $subject.dob|date_format:"%d" == $date.id} selected {/if}>{$date.date}</option>
                            {/foreach}
                        </select>
                        <select name="dob_year">
                            {foreach $years as $year}
                                <option value="{$year.id}" {if $subject.dob|date_format:"%Y" == $year.id} selected {/if}>{$year.year}</option>
                            {/foreach}
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select name="dod_month">
                            {foreach $months as $month}
                                <option value="{$month.id}" {if $subject.dod|date_format:"%m" == $month.id} selected {/if}>{$month.month}</option>
                            {/foreach}
                        </select>
                        <select name="dod_date">
                            {foreach $dates as $date}
                                <option value="{$date.id}" {if $subject.dod|date_format:"%d" == $date.id} selected {/if}>{$date.date}</option>
                            {/foreach}
                        </select>
                        <select name="dod_year">
                            {foreach $years as $year}
                                <option value="{$year.id}" {if $subject.dod|date_format:"%Y" == $year.id} selected {/if}>{$year.year}</option>
                            {/foreach}
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select name="dom_month">
                            {foreach $months as $month}
                                <option value="{$month.id}" {if $subject.dom|date_format:"%m" == $month.id} selected {/if}>{$month.month}</option>
                            {/foreach}
                        </select>
                        <select name="dom_date">
                            {foreach $dates as $date}
                                <option value="{$date.id}" {if $subject.dom|date_format:"%d" == $date.id} selected {/if}>{$date.date}</option>
                            {/foreach}
                        </select>
                        <select name="dom_year">
                            {foreach $years as $year}
                                <option value="{$year.id}" {if $subject.dom|date_format:"%Y" == $year.id} selected {/if}>{$year.year}</option>
                            {/foreach}
                        </select>
                    </td>
                </tr>
    ...
    <input type="hidden" name="hidden_father_id" id="hidden_father_id" value="0" />
    <input type="hidden" name="hidden_mother_id" id="hidden_mother_id" value="0" />
    <input type="hidden" name="hidden_spouse_id" id="hidden_spouse_id" value="0" />
...
<div id="subject_option_container">
    ...
    <div class="section_container">
        <div>
            <center>
                <input type="button" class="save_cancel_button" value="Add" onclick="checkForm()" />
                <input type="button" class="save_cancel_button" value="Cancel" onclick="location.href='/'" />
            </center>
        </div>
    </div>
</div>
